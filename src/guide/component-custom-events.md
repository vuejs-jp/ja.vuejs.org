...
[プロパティの形式](/guide/component-props.html#プロパティの形式-キャメルケース-vs-ケバブケース) と同じように、DOM 内テンプレートを使っている場合は、ケバブケース（kebab-case）のイベントリスナを使うことをおすすめします。文字列テンプレートを使っている場合は、この制約は適用されません。
...
::: tip
コンポーネントの動作を実証するために、全ての発行されたイベントを定義することをおすすめします。
:::
...
フォーム入力バインディングについて学習していたときに、 `v-model` に [組み込み修飾子](/guide/forms.html#修飾子) - `.trim`、`.number`、および `.lazy` があることがわかりました。ただし、場合によっては、独自のカスタム修飾子を追加することもできます。
